...
from i18n import _
import os, sys, errno, stat, getpass, pwd, grp, tempfile, tty
...
#    return st.st_uid == os.getuid() (posix original)
    return True
...
def termwidth():
	if os.isatty(sys.stdin.fileno()) and os.isatty(sys.stdout.fileno()):
		# save console state
		old_settings = tty.tcgetattr(sys.stdout.fileno())
		
		tty.setraw(sys.stdout)
		escseq = "\x9b q"
		sys.stdout.write(escseq)
		result = ""
		while 1:
		...
		tty.tcsetattr(sys.stdout.fileno(), tty.TCSANOW, old_settings)
